...
mpg <- mpg %>%
  mutate(fuel_type = ifelse(fl %in% c("d","p","r"), "petrol", "alternative energy"))
```
...
```{r}
mpg <- mpg %>%
  mutate(trans_type = substr(str_split(trans, "\\(", simplify = TRUE)[,2],1,2)) %>%
...
```{r}
as.Date("2001-08-30") - as.Date("1998-04-22")
```
